...
const Sidebar = () => {
  ...
  return (
    <main>
      {/* Desktop Sidebar */}
      <nav className="hidden md:flex flex-col w-64 bg-white dark:bg-neutral-900 mt-2
         shadow-md dark:shadow-sky-500 rounded-lg fixed h-dvh overflow-y-auto p-6 justify-between dark:text-white">
        <div>
          <ul className="space-y-6">
            {sidebarItems.map(({ label, icon: Icon, path, className }) => {
              ...
              return (
                <li key={label}>
                  <Link to={path} className={`${className ?? ''} ${navStyles(active)}`}>
                    <Icon size={20} />
                    {label}
                  </Link>
                </li>
          ...
          <ul className="border-t py-6 space-y-6">
            {bottomSidebarItems.map(({ label, icon: Icon, path }) => {
              ...
              return (
                <li key={label}>
                  <Link to={path} className={navStyles(active)}>
                    <Icon size={20} />
                    {label}
                  </Link>
                </li>
      ...
      <button
        className="fixed bottom-6 right-6 z-50 bg-neutral-800 cursor-pointer
         dark:bg-neutral-50 p-3 rounded-full shadow-md md:hidden"
